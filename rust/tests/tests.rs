...
mod tests {
    ...
    fn multi_word_completions() {
        ...
        let mut a: Vec<(String, String)> = mit.map(|m| (m.get_value_as_string(), m.matched_string()))
            .collect();
        a.sort();
    ...
    fn fuzzy_completions() {
        ...
        let mit = dictionary::Dictionary::new("test_data/fuzzy.kv")
            .unwrap()
            .get_fuzzy_completions("aafcül", 3);
        let mut a: Vec<(String, String)> = mit.map(|m| (m.get_value_as_string(), m.matched_string()))
            .collect();
        a.sort();
    ...
    fn fuzzy_completions_with_score() {
        ...
        let mit = dictionary::Dictionary::new("test_data/fuzzy.kv")
            .unwrap()
            .get_fuzzy_completions("aafcül", 3);
        let mut a: Vec<(String, String, String)> = mit.map(|m| {
            (
    ...
    fn dictionary_parallel_test() {
        ...
        let sequential_values: Vec<Value> = keys.iter()
            .map(|key| dictionary.get(key).get_value())
            .collect();
        let parallel_values: Vec<Value> = keys.par_iter()
            .map(|key| dictionary.get(key).get_value())
            .collect();
